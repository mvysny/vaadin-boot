...
COPY . /app/
WORKDIR /app/
RUN --mount=type=cache,target=/root/.gradle,sharing=locked --mount=type=cache,target=/root/.vaadin,sharing=locked ./gradlew clean testapp:build -Pvaadin.productionMode --no-daemon --no-watch-fs -x test
